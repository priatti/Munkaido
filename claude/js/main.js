// ===== BEGIN: loadLastValues (FRISSÍTETT VERZIÓ) =====
(function(){
  function prefillFromLast(target) {
    try {
      ...
      // Az "Indítás" (live) fül mezőinek feltöltése
      if (target === 'live') {
      ...
      // A "Teljes nap" (full) fül mezőinek feltöltése
      } else {
...
function showTab(tabName) {
    ...
    if (mainTabs.includes(tabName)) {
        ...
        if (tabName !== 'settings' && tabName !== 'help') {
...
function renderStartTab() {
    const i18n = translations[currentLang];
    const activeShift = window.activeShift || null;

...
// ILLESZD BE EZT A TELJES FÜGGVÉNYT A RÉGI HELYÉRE
...
function addCrossingRow(from = '', to = '', time = '') {
    ...
    // Csak akkor próbáljuk meg kitölteni, ha a gombbal adunk hozzá új sort
    // (azaz a 'from' paraméter üresen érkezik)
    if (from === '') {
        ...
        if (allCrossingRows.length > 0) {
            ...
            // Pontosított selector a 'to' mezőre
    ...
    // Ha a 'time' paraméter üres (azaz gombnyomásra jön létre a sor), akkor az aktuális időt használjuk
    ...
    if (allCrossingRows.length > 0 && from === '') {
        const lastRow = allCrossingRows[allCrossingRows.length - 1];
        const lastToInput = lastRow.querySelector('input.crossing-to, input[placeholder="Hova"], input[placeholder="Nach"]');
...
function renderRecords() {
    // FALLBACK_LOAD_LOCAL_WORKRECORDS: if records is empty, try localStorage to avoid blank list
    try {
    ...
    } catch(e) { console.warn('fallback load workRecords failed', e); }
    const i18n = translations[currentLang];

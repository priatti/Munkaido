...
// ===== BEGIN: loadLastValues (prefill from last closure) =====
(function(){
  function prefillFromLast(target){
    try {
      if (typeof getLatestRecord !== 'function') return;
      var last = getLatestRecord();
      if (!last) return;

      if (target === 'live') {
        var w = document.getElementById('liveWeeklyDriveStart');
        if (w && last.weeklyDriveEndStr) { w.value = last.weeklyDriveEndStr; }
        var k = document.getElementById('liveStartKm');
        if (k && typeof last.kmEnd !== 'undefined' && last.kmEnd !== null) { k.value = String(last.kmEnd); }
      } else {
        var w2 = document.getElementById('weeklyDriveStart');
        if (w2 && last.weeklyDriveEndStr) { w2.value = last.weeklyDriveEndStr; }
        var k2 = document.getElementById('kmStart');
        if (k2 && typeof last.kmEnd !== 'undefined' && last.kmEnd !== null) { k2.value = String(last.kmEnd); }
      }
    } catch (e) { console.warn('prefillFromLast error', e); }
  }

  // Define/override global function
  window.loadLastValues = function(isStartLive){
    // If the toggles hide fields, we still try to prefill safely when present
    prefillFromLast(isStartLive ? 'live' : 'full');
  };
})();
// ===== END: loadLastValues (prefill from last closure) =====

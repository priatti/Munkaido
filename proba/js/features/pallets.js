...
function generatePalletReport() {
    ...
    container.innerHTML = types.map(type => `
        <label class="pallet-report-label flex items-center justify-between p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer transition-colors">
            <span class="font-semibold text-gray-800 dark:text-gray-100">${type}</span>
            <input type="checkbox" value="${type}" class="pallet-report-checkbox h-5 w-5 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" checked onchange="togglePalletReportLabel(this)">
        </label>
    `).join('');
    
    document.querySelectorAll('.pallet-report-checkbox').forEach(togglePalletReportLabel);

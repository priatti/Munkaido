const translations = {
    hu: {
        ...
        settingsSyncDesc: "Jelentkezz be a Google fiókodpal, hogy az adataidat a felhőbe mentsd és több eszközön is elérd.",
...
function discardShift() { 
    const confirmMessage = currentLang === 'de' 
        ? 'Möchten Sie den Arbeitstag wirklich verwerfen?' 
        : translations[currentLang].alertConfirmDelete;
...
function exportToPDF() {
    try {
        if (!currentMonthlyData) { alert("Először generálj riportot!"); return; }
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF('p', 'mm', 'a4');
        const userName = document.getElementById('userNameInput').value || (currentLang === 'de' ? 'Name nicht angegeben' : 'Név Nincs Megadva');
        ...
        const errorMessage = currentLang === 'de' 
            ? "Fehler beim Erstellen der PDF: " + e.message
            : "Hiba történt a PDF generálása közben: " + e.message;
        alert(errorMessage);
    }
}
...
function importData() {
    ...
    reader.onload = function (e) {
        ...
        } catch (err) { showCustomAlert('Hiba: ' + err.message, 'info'); }
    };
...
async function runNightWorkRecalculation() {
    ...
    console.log('Éjszakai idők újraszámítása a 20:00-05:00 szabály szerint...');
    let updatedCount = 0;
    ...
    console.log(`${updatedCount} bejegyzés frissítve.`);
    renderApp();
}
...
function updatePalletBalance() {
    ...
    const quantityUnit = currentLang === 'de' ? 'Stk' : 'db';

    displayEl.innerHTML = `
        <p class="text-sm font-medium">${i18n.palletsBalance}</p>
        <p class="text-2xl font-bold ${colorClass}">${balance} ${quantityUnit}</p>
    `;
}
...
function renderPalletRecords() {
    ...
    container.innerHTML = sortedRecords.map(p => {
        ...
        const licensePlateLabel = currentLang === 'de' ? 'Kennzeichen:' : 'Rendszám:';
        const quantityUnit = currentLang === 'de' ? 'Stk' : 'db';
        const licensePlateHTML = p.licensePlate
            ? `<p class="text-xs text-gray-400 mt-1">${licensePlateLabel} ${p.licensePlate}</p>`
            : '';
        ...
        <div class="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-sm border-l-4 ${actionClass} flex items-center justify-between">
            ...
            <div class="text-right">
                 <p class="font-bold text-lg ${textColor}">${sign}${p.quantity} ${quantityUnit}</p>
...
function generatePalletReport() {
    const noDataMessage = currentLang === 'de' 
        ? "Keine Palettenbewegungen für einen Bericht vorhanden."
        : "Nincs raklapmozgás, amiből riportot lehetne készíteni.";
    
    if (palletRecords.length === 0) {
        showCustomAlert(noDataMessage, "info");
        return;
    }
    try {
        const { jsPDF } = window.jspdf;
        ...
        const reportTitle = currentLang === 'de' ? 'Palettenbewegung Bericht' : 'Raklapmozgás Riport';
        const currentBalanceLabel = currentLang === 'de' ? 'Aktueller Saldo:' : 'Aktuális egyenleg:';
        
        doc.setFontSize(18); doc.setFont(undefined, 'bold'); doc.text(reportTitle, 105, 15, { align: 'center' });
        doc.setFontSize(12); doc.text(userName, 105, 23, { align: 'center' });
        doc.setFontSize(10); doc.text(`${currentBalanceLabel} ${currentBalance} ${currentLang === 'de' ? 'Stk' : 'db'}`, 105, 31, { align: 'center' });

        let yPos = 40;
        const headers = currentLang === 'de' 
            ? ['Datum', 'Standort', 'Vorgang', 'Menge', 'Kennzeichen', 'Saldo']
            : ['Dátum', 'Helyszín', 'Művelet', 'Mennyiség', 'Rendszám', 'Egyenleg'];
        ...
        sortedRecords.forEach((p, index) => {
            ...
            if (currentLang === 'de') {
                switch(p.action) {
                    case 'felvett': actionText = 'Aufgenommen'; break;
                    case 'leadott': actionText = 'Abgegeben'; break;
                    case 'csere': actionText = '1:1 Tausch'; break;
                }
            } else {
                switch(p.action) {
            ...
            const quantityUnit = currentLang === 'de' ? 'Stk' : 'db';
            const row = [p.date, p.location, actionText, `${p.quantity} ${quantityUnit}`, p.licensePlate || '-', `${balanceAfterEach[index]} ${quantityUnit}`];
            row.forEach((cell, i) => {
        ...
        const fileName = currentLang === 'de' 
            ? `Paletten_Bericht-${userName.replace(/ /g, "_")}-${new Date().toISOString().split("T")[0]}.pdf`

...
function editPalletEntry(id) {
    ...
    const title = document.getElementById('palletsTitle');
    ...
    if (editingPalletId) {
        ...
        if (recordIndex > -1) {
            palletRecords[recordIndex] = {
                ...palletRecords[recordIndex],
                date, location, palletsGiven, palletsTaken,
...
function renderPalletRecords() {
    ...
    const i18n = translations[currentLang];
    const container = safeGetElement('palletRecordsContainer');
    if (!container) return;
...
function renderPalletTypeSelector() {
    ...
    if (!container) {
        console.warn('pallet-type-selector not found');
        return;
    }
...
function generatePalletReport() {
    ...
    container.innerHTML = types.map(type => `
        <label class="pallet-report-label flex items-center justify-between p-4 rounded-lg border-2 cursor-pointer transition-all duration-200 transform hover:scale-[1.02]">
            <span class="font-bold text-lg">${type}</span>
            <input type="checkbox" value="${type}" class="pallet-report-checkbox h-6 w-6 rounded border-2 border-gray-400 text-green-600 focus:ring-2 focus:ring-green-500" checked>
        </label>
    `).join('');
...
function togglePalletReportLabel(checkbox) {
    ...
    if (checkbox.checked) {
        label.classList.add(
            ...
            'dark:border-green-400', 'shadow-lg', 'shadow-green-200/50'
        );
        if (!span.textContent.startsWith('✓')) {
            span.textContent = '✓ ' + span.textContent;
            span.classList.add('text-green-800', 'dark:text-green-200');
        }
        checkbox.classList.add('text-green-600');
    } else {
        label.classList.add('border-gray-200', 'dark:border-gray-600', 'bg-gray-50', 'dark:bg-gray-800');
        if (span.textContent.startsWith('✓')) {
            span.textContent = span.textContent.substring(2);
        }
        span.classList.remove('text-green-800', 'dark:text-green-200');
...
function createSelectedPalletReport(selectedTypesArray = null) {
    ...
    let selectedTypes = selectedTypesArray || Array.from(document.querySelectorAll('.pallet-report-checkbox:checked')).map(cb => cb.value);
    ...
    try {
        ...
        selectedTypes.forEach((type, typeIndex) => {
            ...
            doc.setFontSize(10);
            let xPos = 15;
            headers.forEach((header, i) => { doc.text(header, xPos, yPos); xPos += colWidths[i]; });
            yPos += 2;
            ...
            typeRecords.forEach(record => {
                if (yPos > pageBottom - 10) { doc.addPage(); yPos = 20; }
                runningBalance += (record.palletsTaken || 0) - (record.palletsGiven || 0);
                const row = [record.date, record.location, (record.palletsGiven || 0).toString(), (record.palletsTaken || 0).toString(), record.licensePlate || '-', `${runningBalance >= 0 ? '+' : ''}${runningBalance}`];
                xPos = 15;
                row.forEach((cell, i) => {

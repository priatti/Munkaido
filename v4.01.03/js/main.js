...
async function handleEmailLoginSubmit() {
  ...
  try {
    ...
    switch(error.code) {
      case 'auth/user-not-found': errorMsg = '❌ Nincs ilyen felhasználó. Regisztrálj!'; break;
      case 'auth/wrong-password': errorMsg = '❌ Helytelen jelszó'; break;
      case 'auth/invalid-email': errorMsg = '❌ Érvénytelen email formátum'; break;
      case 'auth/too-many-requests': errorMsg = '⏳ Túl sok próbálkozás. Próbáld újra később.'; break;
      case 'auth/user-disabled': errorMsg = '🚫 Ez a fiók le lett tiltva'; break;
      case 'auth/network-request-failed': errorMsg = '📡 Nincs internetkapcsolat'; break;
    }
...
async function handleEmailRegistrationSubmit() {
  ...
  try {
    ...
    let errorMsg = 'Ismeretlen hiba történt';
    switch(error.code) {
      case 'auth/email-already-in-use': errorMsg = '❌ Ez az email cím már használatban van'; break;
      case 'auth/invalid-email': errorMsg = '❌ Érvénytelen email formátum'; break;
      case 'auth/weak-password': errorMsg = '❌ Túl gyenge jelszó. Használj legalább 6 karaktert!'; break;
      case 'auth/network-request-failed': errorMsg = '📡 Nincs internetkapcsolat'; break;
    }
...
document.getElementById('forgot-password-link')?.addEventListener('click', async (e) => {
  ...
  try {
    ...
    switch(error.code) {
      case 'auth/user-not-found': errorMsg = '❌ Nincs ilyen email cím regisztrálva'; break;
      case 'auth/invalid-email': errorMsg = '❌ Érvénytelen email formátum'; break;
      default: errorMsg = '❌ Nem sikerült elküldeni az emailt. Próbáld újra később.';
    }
    showCustomAlert(errorMsg, 'info');
  }
});

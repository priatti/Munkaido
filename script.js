document.addEventListener('DOMContentLoaded', () => {
    ...
    const translations = {
        hu: {
            ...
            earliestStartDisclaimer: "Figyelem: A kalkuláció a napi pihenőidővel számol, a heti pihenőidőt nem veszi figyelembe.",
            // Live nézet
        ...
        de: {
            ...
            menuMore: "Mehr",
            menuHelp: "Hilfe & Info",
            menuHelpDesc: "Wie die App funktioniert",
            ...
            // Legkorábbi indulás
            earliestStartTitle: "Früheste Abfahrt",
            with9hRest: "Mit 9-Std. Ruhezeit",
            reduced: "(verkürzt)",
            with11hRest: "Mit 11-Std. Ruhezeit",
            regular: "(regulär)",
            noMoreReducedRests: "Keine verkürzten Ruhezeiten mehr verfügbar.",
            earliestStartDisclaimer: "Achtung: Die Berechnung bezieht sich auf die tägliche Ruhezeit und berücksichtigt nicht die wöchentliche Ruhezeit.",
            // Live-Ansicht
    ...
    // A DOMContentLoaded eseménykezelő már a szkript elején van, így ez a rész idekerül
    initTheme();
    loadSettings();
        ...
        if (allowance.remainingRests > 0) {
            ...
            <div class="bg-indigo-50 dark:bg-indigo-900/50 border border-indigo-200 dark:border-indigo-800 rounded-lg p-3 space-y-2">
                ...
                <div class="text-sm text-gray-700 dark:text-gray-200">
                    <strong>${i18n.with9hRest}</strong> ${i18n.reduced}: 
                    <div class="font-bold text-lg text-indigo-600 dark:text-indigo-300">${formatDateTime(startTime9h)}</div>
                </div>
                <div class="text-sm text-gray-700 dark:text-gray-200">
                    <strong>${i18n.with11hRest}</strong> ${i18n.regular}: 
                    <div class="font-bold text-lg">${formatDateTime(startTime11h)}</div>
                </div>
                <p class="text-xs text-gray-500 dark:text-gray-400 italic pt-2 border-t border-indigo-200/50">${i18n.earliestStartDisclaimer}</p>
            </div>`;
        } else {
            htmlContent = `
            <div class="bg-yellow-50 dark:bg-yellow-900/50 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3">
                 ...
                 <p class="text-xs text-gray-500 dark:text-gray-400 italic pt-2 mt-2 border-t border-yellow-200/50">${i18n.earliestStartDisclaimer}</p>
            </div>`;
        }

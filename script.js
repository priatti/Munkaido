const translations = {
    ...
    de: {
        ...
        helpRecordingLiveTitle: "Live-Erfassung (Registerkarte 'Start')",
        ...
        helpRecordingFullTitle: "Nachträgliche Erfassung (Registerkarte 'Ganzer Tag')",
        ...
        helpSectionUsingApp: "Nutzung der App-Funktionen",
        helpListTitle: "Einträge verwalten (Registerkarte 'Liste')",
        helpListDesc: "Auf der Registerkarte 'Liste' sehen Sie alle Ihre bisher gespeicherten Arbeitstage in chronologischer Reihenfolge, die neuesten zuerst. Bei jedem Eintrag sehen Sie die wichtigsten Daten: Start- und Endort und -zeit, Gesamtarbeitszeit, Lenkzeit und die zurückgelegte Strecke. Wenn Sie einen früheren Tag bearbeiten möchten, verwenden Sie das Stift-Symbol (✏️). Wenn Sie einen Eintrag löschen möchten, klicken Sie auf das Mülleimer-Symbol (🗑️).",
        helpReportTitle: "Monatsbericht erstellen und teilen (Registerkarte 'Bericht')",
        helpReportDesc: "Diese Funktion erstellt eine druckbare, offizielle monatliche Arbeitszeitabrechnung aus Ihren Daten. Besonders nützlich, wenn Sie Ihre Arbeitszeitnachweise einreichen müssen.<br>1. <strong>Vorbereitungen:</strong> Um einen Bericht zu erstellen, müssen Sie Ihren vollständigen Namen im Menü <strong>Einstellungen</strong> eingeben.<br>2. <strong>Monat auswählen:</strong> Wählen Sie auf der Registerkarte 'Bericht' den Monat aus, für den Sie eine Abrechnung erstellen möchten.<br>3. <strong>Erstellen:</strong> Klicken Sie auf die Schaltfläche \"Bericht erstellen\".<br>4. <strong>Herunterladen oder Teilen:</strong> Nach erfolgreicher Erstellung erscheinen die Schaltflächen \"PDF herunterladen\" und \"PDF teilen\". Mit der Teilen-Funktion können Sie das erstellte PDF direkt per WhatsApp, Messenger, E-Mail oder einer anderen Anwendung versenden.",
        helpSectionPWA: "Installation der Anwendung (PWA)",
        helpPwaBenefitsTitle: "Warum lohnt sich die Installation?",
        helpPwaBenefitsDesc: "<li><strong>Schneller Zugriff:</strong> Nach der Installation hat die App ein eigenes Symbol auf Ihrem Startbildschirm, sodass Sie sie mit einem einzigen Fingertipp starten können.</li><li><strong>Offline-Betrieb:</strong> Die installierte App startet auch ohne Internetverbindung, und ihre wichtigsten Funktionen bleiben verfügbar.</li><li><strong>App-ähnliches Erlebnis:</strong> Die installierte Version wird ohne die störenden Elemente des Browsers (Adressleiste, Menüs) angezeigt.</li>",
        helpPwaInstallTitle: "Wie wird installiert?",
        helpPwaInstallDesc: "Wenn die App noch nicht installiert ist, erscheint im Menü <strong>Einstellungen</strong> eine Karte \"App installieren\" mit einer Schaltfläche. Wenn diese nicht sichtbar ist, können Sie die Installation auch manuell starten:<br><li><strong>Desktop (Chrome/Edge):</strong> Suchen Sie in der Adressleiste des Browsers nach einem Installationssymbol.</li><li><strong>Android (Chrome):</strong> Wählen Sie im Browsermenü (drei Punkte) die Option \"App installieren\" oder \"Zum Startbildschirm hinzufügen\".</li><li><strong>iPhone/iPad (Safari):</strong> Tippen Sie auf das Teilen-Symbol und wählen Sie dann im Menü \"Zum Home-Bildschirm\".</li>",
...
window.addEventListener('beforeinstallprompt', (e) => {
  // Megakadályozza a böngésző alapértelmezett telepítési felhívásának megjelenését
  e.preventDefault();
  // Elmenti az eseményt későbbi használatra
  deferredPrompt = e;
  // Megjeleníti a saját telepítési gombunkat tartalmazó felületet
  if (installContainer) {
...
if (installButton) {
  installButton.addEventListener('click', async () => {
    ...
    // Megjeleníti a böngésző natív telepítési ablakát
    deferredPrompt.prompt();
    // Vár a felhasználó válaszára
    const { outcome } = await deferredPrompt.userChoice;
    console.log(`User response to the install prompt: ${outcome}`);
    // Az eseményt csak egyszer lehet felhasználni, ezért töröljük
    deferredPrompt = null;
    // Elrejti a saját gombunkat, mivel a telepítés megtörtént vagy el lett utasítva
    if (installContainer) {
...
window.addEventListener('appinstalled', () => {
  // Ha az alkalmazást sikeresen telepítették, végleg eltüntetjük a gombot
  deferredPrompt = null;
  if (installContainer) {
    installContainer.classList.add('hidden');
  }
...
function applyFeatureToggles() {
    ...
    if (tachoHelperDisplay) {
        tachoHelperDisplay.style.display = showDriveTime ? 'block' : 'none';
    }
    if (liveAllowanceDisplay) {
        liveAllowanceDisplay.style.display = showDriveTime ? 'block' : 'none';
    }
    if (tachographMenuItem) {
        tachographMenuItem.style.display = showDriveTime ? 'flex' : 'none';
    }

    const activeTachographContent = document.getElementById('content-tachograph');
    if (!showDriveTime && activeTachographContent && !activeTachographContent.classList.contains('hidden')) {
        showTab('live');
    }
...
function renderTachoHelperCards() {
    ...
    if (inProgressEntry) {
        ...
        // Render "Earliest Start" card (original logic)

...
// Töröljük az importot a main.js-ből!
// A szükséges adatokat (i18n, records) mostantól paraméterként kapja meg.

//======================================================================
// ===== SEGÉDFÜGGVÉNYEK (csak ehhez a modulhoz) =======================
//======================================================================
function formatDuration(i18n, currentLang, minutes) {
...
//======================================================================
function calculateRestDebt(records) {
...
function calculateWeeklyAllowance(records) {
    const now = new Date();
...
export function renderWeeklyAllowance(i18n, currentLang, records) {
    ...
    const allowance = calculateWeeklyAllowance(records);
    const debtMinutes = calculateRestDebt(records);
    const debtHTML = debtMinutes > 0 ? `
        <div class="p-2 bg-red-50 dark:bg-red-600/20 rounded-lg border border-red-200 dark:border-red-800">
            ...
            <p class="text-2xl font-bold text-red-600 dark:text-red-400">${formatDuration(i18n, currentLang, debtMinutes)}</p>
        </div>` : '';
    const html = `
...
export function handleTachographToggle(i18n, currentLang, records, checkbox, recordId, type) {
    ...
    renderTachographAnalysis(i18n, currentLang, records);
    renderWeeklyAllowance(i18n, currentLang, records);
}

export function renderTachographAnalysis(i18n, currentLang, records) {
            ...
            if (restDurationHours >= 24) {
                ...
                if (isMarkedAsWeekly) {
                    if (restDurationHours >= 45) {
                        restAnalysis = { text: `${i18n.tachoRegularWeeklyRest} (${formatDuration(i18n, currentLang, restDurationMinutes)})`, colorClass: 'bg-green-700 text-white' };
                    } else {
                        restAnalysis = { text: `${i18n.tachoReducedWeeklyRest} (${formatDuration(i18n, currentLang, restDurationMinutes)})`, colorClass: 'bg-green-700 text-white' };
                    }
                } else {
                     restAnalysis = { text: `${i18n.tachoLongRest} (${formatDuration(i18n, currentLang, restDurationMinutes)})`, colorClass: 'bg-gray-300 text-gray-800' };
                }
            } else if (restDurationHours < 9) {
                restAnalysis = { text: `${i18n.tachoIrregularRest} (${formatDuration(i18n, currentLang, restDurationMinutes)})`, colorClass: 'bg-red-500 text-white' };
            } else if (isSplitRest) {
                restAnalysis = { text: `${i18n.tachoSplitRest} (${formatDuration(i18n, currentLang, restDurationMinutes)})`, colorClass: 'bg-green-200 text-green-800' };
            } else if (restDurationHours >= 11 && prevWorkDurationHours <= 13) {
                restAnalysis = { text: `${i18n.tachoRegularDailyRest} (${formatDuration(i18n, currentLang, restDurationMinutes)})`, colorClass: 'bg-green-500 text-white' };
            } else {
                ...
                restAnalysis = { text: `${countText} ${i18n.tachoReducedDailyRest}${reason} (${formatDuration(i18n, currentLang, restDurationMinutes)})`, colorClass: colorClass };
            }
        }
        let driveAnalysis;
        const driveHours = currentRecord.driveMinutes / 60;
        if (driveHours > 10) {
            driveAnalysis = { text: `${i18n.tachoIrregularDrive} (${formatDuration(i18n, currentLang, currentRecord.driveMinutes)})`, colorClass: 'bg-red-500 text-white' };
            ...
            driveAnalysis = { text: `${countInWeek}. ${countInWeek > 2 ? '(irregular) ' : ''}${i18n.tachoIncreasedDrive} (${formatDuration(i18n, currentLang, currentRecord.driveMinutes)})`, colorClass: countInWeek > 2 ? 'bg-red-500 text-white' : 'bg-blue-400 text-white' };
        } else {
            driveAnalysis = { text: `${i18n.tachoNormalDrive} (${formatDuration(i18n, currentLang, currentRecord.driveMinutes)})`, colorClass: 'bg-gray-300 text-gray-800' };
        }
        ...
        if (res.rest.duration >= (24 * 60)) {
            weeklyRestCheckboxHTML = `
                <label for="weekly-${res.record.id}" class="flex items-center gap-2 cursor-pointer text-xs p-1 rounded-md ${res.isWeekly ? 'bg-green-200 dark:bg-green-800' : 'bg-gray-200 dark:bg-gray-600'}">
                   <input type="checkbox" id="weekly-${res.record.id}" onchange="window.app.handleTachographToggle(this, '${res.record.id}', 'weekly')" ${res.isWeekly ? 'checked' : ''} class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg p-3 shadow-sm border-l-4 ${res.rest.colorClass.replace('bg-', 'border-').replace(/ text-\w+-?\d*/g, '')}">
            <div class="flex justify-between items-start mb-3">
                ...
                <label for="split-${res.record.id}" class="flex items-center gap-2 cursor-pointer p-2 rounded-lg transition-colors ${isSplitActiveClass}">
                   <input type="checkbox" id="split-${res.record.id}" onchange="window.app.handleTachographToggle(this, '${res.record.id}', 'split')" ${res.isSplit ? 'checked' : ''} class="h-5 w-5 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">

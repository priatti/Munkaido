const translations = {
    hu: {
		...
		weekendTransitionTitle: "Vasárnap-hétfő átmenet",
		weekendTransitionDesc: "A tachográf vasárnap éjfélkor nullázódott. Kérem adja meg a tényleges napi vezetési időt.",
		actualDailyDriveTime: "Tényleges napi vezetési idő",
    },
    de: {
        // Allgemein
		...
		weekendTransitionTitle: "Sonntag-Montag Übergang",
		weekendTransitionDesc: "Der Tachograph wurde Sonntag Mitternacht zurückgesetzt. Bitte geben Sie die tatsächliche tägliche Lenkzeit ein.",
		actualDailyDriveTime: "Tatsächliche tägliche Lenkzeit",
    }
};
...
async function saveEntry() {
	...
	const isWeekendCrossing = isWeekendTransition(recordData.date, recordData.date, recordData.startTime, recordData.endTime);

	if (!isWeekendCrossing && parseTimeToMinutes(recordData.weeklyDriveEndStr) > 0 && parseTimeToMinutes(recordData.weeklyDriveEndStr) < parseTimeToMinutes(recordData.weeklyDriveStartStr)) { 
		showCustomAlert(i18n.alertWeeklyDriveEndLower, 'info'); 
		return; 
	}
	
	if (isWeekendCrossing && parseTimeToMinutes(recordData.weeklyDriveEndStr) < parseTimeToMinutes(recordData.weeklyDriveStartStr)) {
		showWeekendTransitionPopup(recordData);
		return;
	}
...
// ==========================================================
// ===== VASÁRNAP-HÉTFŐ ÁTMENET KEZELÉSE (v8.051) ============
// ==========================================================

function isWeekendTransition(startDate, endDate, startTime, endTime) {
    const start = new Date(`${startDate}T${startTime}`);
    const end = new Date(`${endDate}T${endTime}`);
    return start.getDay() === 0 && end.getDay() === 1; // vasárnap -> hétfő
}

let pendingWeekendRecord = null;

function showWeekendTransitionPopup(recordData) {
    pendingWeekendRecord = recordData;
    const overlay = document.getElementById('weekend-transition-overlay');
    overlay.classList.remove('hidden');
    overlay.classList.add('flex');
    document.getElementById('actualDailyDriveInput').value = '';
    document.getElementById('actualDailyDriveInput').focus();
}

function hideWeekendTransitionPopup() {
    const overlay = document.getElementById('weekend-transition-overlay');
    overlay.classList.add('hidden');
    overlay.classList.remove('flex');
    pendingWeekendRecord = null;
}

function cancelWeekendTransition() {
    hideWeekendTransitionPopup();
}

async function confirmWeekendTransition() {
    const actualDriveInput = document.getElementById('actualDailyDriveInput').value.trim();
    const actualDriveMinutes = parseTimeToMinutes(actualDriveInput);
    
    if (actualDriveMinutes <= 0) {
        showCustomAlert('Kérem adja meg a tényleges napi vezetési időt!', 'info');
        return;
    }
    
    hideWeekendTransitionPopup();
    
    const compensationMinutes = parseTimeToMinutes(pendingWeekendRecord.compensationTime) || 0;
    const grossWorkMinutes = calculateWorkMinutes(pendingWeekendRecord.startTime, pendingWeekendRecord.endTime);

    const newRecord = {
        ...pendingWeekendRecord,
        id: editingId || String(Date.now()),
        workMinutes: Math.max(0, grossWorkMinutes - compensationMinutes),
        compensationMinutes: compensationMinutes,
        nightWorkMinutes: calculateNightWorkMinutes(pendingWeekendRecord.startTime, pendingWeekendRecord.endTime),
        driveMinutes: Math.max(0, parseTimeToMinutes(pendingWeekendRecord.weeklyDriveEndStr) - parseTimeToMinutes(pendingWeekendRecord.weeklyDriveStartStr)),
        actualDailyDriveMinutes: actualDriveMinutes,
        kmDriven: Math.max(0, pendingWeekendRecord.kmEnd - pendingWeekendRecord.kmStart)
    };
    ...
    await saveRecord(newRecord);
    showCustomAlert(translations[currentLang].alertSaveSuccess, 'success', () => {
        if (inProgressEntry) {
            localStorage.removeItem('inProgressEntry');
            inProgressEntry = null;
        }

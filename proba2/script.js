...
async function saveEntry() {
    ...
    if ((newRecord.driveMinutes === 0 || newRecord.kmDriven === 0) && !editingId) {
...
function renderSummary() {
    ...
    const periods = [
        { labelKey: 'summaryToday', data: calculateSummaryForDateRange(getDayRange(now)) },
        { labelKey: 'summaryYesterday', data: calculateSummaryForDateRange(getDayRange(now, -1)) },
        { labelKey: 'summaryThisWeek', data: calculateSummaryForDateRange(getWeekRange(now)) },
        { labelKey: 'summaryLastWeek', data: calculateSummaryForDateRange(getWeekRange(now, -1)) },
        { labelKey: 'summaryThisMonth', data: calculateSummaryForMonth(now) },
        { labelKey: 'summaryLastMonth', data: calculateSummaryForMonth(now, -1) }
    ];

    container.innerHTML = periods.map(p => {
        if (p.data.count === 0) {
            return `<div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg shadow-sm"><h3 class="font-semibold mb-2">${i18n[p.labelKey]}</h3><p class="text-gray-500">${i18n.summaryNoData}</p></div>`;
        }
        return `
        <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm">
            <h3 class="font-semibold mb-2">${i18n[p.labelKey]} <span class="text-sm font-normal text-gray-500">(${p.data.count} ${i18n.summaryDays})</span></h3>
            <div class="grid grid-cols-2 gap-2 text-sm">
                <div>${i18n.summaryWork}: <span class="font-bold float-right">${formatDuration(p.data.workMinutes)}</span></div>
                <div>${i18n.summaryNight}: <span class="font-bold float-right">${formatDuration(p.data.nightWorkMinutes)}</span></div>
                <div>${i18n.summaryDrive}: <span class="font-bold float-right">${formatDuration(p.data.driveMinutes)}</span></div>
                <div>${i18n.summaryDistance}: <span class="font-bold float-right">${p.data.kmDriven} km</span></div>
            </div>
        </div>`;
    }).join('');
}

function calculateSummaryForDateRange(range) {
    const filteredRecords = records.filter(r => new Date(r.date) >= range.start && new Date(r.date) <= range.end);
    return filteredRecords.reduce((acc, r) => {
        acc.count++;
        acc.workMinutes += r.workMinutes || 0;
        acc.nightWorkMinutes += r.nightWorkMinutes || 0;
        acc.driveMinutes += r.driveMinutes || 0;
        acc.kmDriven += r.kmDriven || 0;
        return acc;
    }, { count: 0, workMinutes: 0, nightWorkMinutes: 0, driveMinutes: 0, kmDriven: 0 });
}

function calculateSummaryForMonth(date, offset = 0) {
    const targetDate = new Date(date.getFullYear(), date.getMonth() + offset, 1);
    const year = targetDate.getFullYear();
    const month = String(targetDate.getMonth() + 1).padStart(2, '0');
    const filteredRecords = records.filter(r => r.date.startsWith(`${year}-${month}`));
    const result = filteredRecords.reduce((acc, r) => {
        acc.workMinutes += r.workMinutes || 0;
        acc.nightWorkMinutes += r.nightWorkMinutes || 0;
        acc.driveMinutes += r.driveMinutes || 0;
        acc.kmDriven += r.kmDriven || 0;
        return acc;
    }, { workMinutes: 0, nightWorkMinutes: 0, driveMinutes: 0, kmDriven: 0 });
    result.count = filteredRecords.length;
    return result;
}

function navigateStats(direction) {
    if (statsView === 'daily') statsDate.setDate(statsDate.getDate() + direction);
    else if (statsView === 'monthly') statsDate.setMonth(statsDate.getMonth() + direction);
    else if (statsView === 'yearly') statsDate.setFullYear(statsDate.getFullYear() + direction);
    renderStats();
}
...
function renderStats() {
    ...
    const buttons = document.querySelectorAll('.stats-view-btn');
    buttons.forEach(btn => btn.classList.remove('active'));
    document.getElementById(`stats-view-${statsView}`).classList.add('active');

    if (statsView === 'daily') {
        data = getDailyData(statsDate);
        periodDisplay.textContent = statsDate.toLocaleDateString(i18n.locale, { year: 'numeric', month: 'long', day: 'numeric' });
    } else if (statsView === 'monthly') {
        data = getMonthlyData(statsDate);
        periodDisplay.textContent = statsDate.toLocaleDateString(i18n.locale, { year: 'numeric', month: 'long' });
    } else { // yearly
        data = getYearlyData(statsDate);
    ...
    const hasData = data && data.datasets && Object.values(data.datasets).some(dataset => dataset.some(val => val > 0));

    if (hasData) {
        noDataEl.classList.add('hidden');
        chartsContainer.classList.remove('hidden');
        
        const workTimeHours = data.datasets.work.reduce((a, b) => a + b, 0);
        const driveTimeHours = data.datasets.drive.reduce((a, b) => a + b, 0);
        const nightTimeHours = data.datasets.night.reduce((a, b) => a + b, 0);
        const kmTotal = data.datasets.km.reduce((a, b) => a + b, 0);

        document.getElementById('workTimeTotal').textContent = formatDuration(workTimeHours * 60);
        document.getElementById('driveTimeTotal').textContent = formatDuration(driveTimeHours * 60);
        document.getElementById('nightTimeTotal').textContent = formatDuration(nightTimeHours * 60);
        document.getElementById('kmTotal').textContent = `${Math.round(kmTotal)} km`;

        createOrUpdateBarChart(workTimeChart, 'workTimeChart', data.labels, data.datasets.work, i18n.statsWorkTime, 'rgba(34, 197, 94, 0.6)');
        createOrUpdateBarChart(driveTimeChart, 'driveTimeChart', data.labels, data.datasets.drive, i18n.statsDriveTime, 'rgba(59, 130, 246, 0.6)');
        createOrUpdateBarChart(nightTimeChart, 'nightTimeChart', data.labels, data.datasets.night, i18n.statsNightTime, 'rgba(139, 92, 246, 0.6)');
        createOrUpdateBarChart(kmChart, 'kmChart', data.labels, data.datasets.km, i18n.statsKmDriven, 'rgba(249, 115, 22, 0.6)');
    } else {
...
function getDailyData(date) {
    ...
    const labels = [date.toLocaleDateString(translations[currentLang].locale, { weekday: 'short' })];
    const datasets = {
        work: record ? [(record.workMinutes || 0) / 60] : [0],
        drive: record ? [(record.driveMinutes || 0) / 60] : [0],
        night: record ? [(record.nightWorkMinutes || 0) / 60] : [0],
        km: record ? [record.kmDriven || 0] : [0]
    };
...
function getMonthlyData(date) {
    ...
    records.filter(r => r.date.startsWith(year)).forEach(r => {
        const monthIndex = new Date(r.date + 'T00:00:00Z').getUTCMonth();
...
function getYearlyData(date) {
    const currentYear = date.getFullYear();
    const years = Array.from({ length: 5 }, (_, i) => currentYear - 4 + i);
    const datasets = { work: Array(5).fill(0), drive: Array(5).fill(0), night: Array(5).fill(0), km: Array(5).fill(0) };

    records.forEach(r => {
        const year = new Date(r.date).getFullYear();
        const yearIndex = years.indexOf(year);
        if (yearIndex !== -1) {
            datasets.work[yearIndex] += (r.workMinutes || 0) / 60;
            datasets.drive[yearIndex] += (r.driveMinutes || 0) / 60;
            datasets.night[yearIndex] += (r.nightWorkMinutes || 0) / 60;
            datasets.km[yearIndex] += r.kmDriven || 0;
        }
    });
    return { labels: years, datasets };
}

function createOrUpdateBarChart(chartInstance, canvasId, labels, data, label, color) {
    ...
    if (chartInstance) {
        ...
        const chart = new Chart(ctx, {
            ...
            options: {
                ...
                scales: {
                    y: { beginAtZero: true, ticks: { color: document.documentElement.classList.contains('dark') ? '#ccc' : '#666' } },
        ...
        if (canvasId === 'workTimeChart') workTimeChart = chart;
        else if (canvasId === 'driveTimeChart') driveTimeChart = chart;
        else if (canvasId === 'nightTimeChart') nightTimeChart = chart;
        else if (canvasId === 'kmChart') kmChart = chart;
    }
}
...
function generateMonthlyReport() {
    ...
    const recordsForMonth = records.filter(r => r.date.startsWith(month));
    const container = document.getElementById('monthlyReportContent');
    
    currentMonthlyData = {
        month: month,
        records: recordsForMonth.sort((a,b) => new Date(a.date) - new Date(b.date))
    };
    
    container.innerHTML = `<div class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4" role="alert"><p class="font-bold">${translations[currentLang].reportPrepared}</p></div>`;
...
function exportData() {
    ...
    link.href = url;
    link.download = `munkaido_pro_backup_${new Date().toISOString().split('T')[0]}.json`;
...
function renderTachographAnalysis() {
    ...
    container.innerHTML = sorted.map((record, index) => {
        ...
        const restBefore = prevRecord ? calculateWorkMinutes(prevRecord.endTime, record.startTime) : null;
        const workTime = record.workMinutes || 0;
        const driveTime = record.driveMinutes || 0;

        let restAnalysis = '';
        if (restBefore !== null) {
            if (restBefore >= 24 * 60) restAnalysis = `<div class="p-2 bg-blue-100 rounded-md text-blue-800">${translations[currentLang].tachoLongRest} (${formatDuration(restBefore)})</div>`;
            else if (restBefore >= 11 * 60) restAnalysis = `<div class="p-2 bg-green-100 rounded-md text-green-800">${translations[currentLang].tachoRegularDailyRest} (${formatDuration(restBefore)})</div>`;
            else if (restBefore >= 9 * 60) restAnalysis = `<div class="p-2 bg-yellow-100 rounded-md text-yellow-800">${translations[currentLang].tachoReducedDailyRest} (${formatDuration(restBefore)})</div>`;
            else restAnalysis = `<div class="p-2 bg-red-100 rounded-md text-red-800">${translations[currentLang].tachoIrregularRest} (${formatDuration(restBefore)})</div>`;
        }

        let driveAnalysis = '';
        if (driveTime > 10 * 60) driveAnalysis = `<div class="p-2 bg-red-100 rounded-md text-red-800">${translations[currentLang].tachoIrregularDrive} (${formatDuration(driveTime)})</div>`;
        else if (driveTime > 9 * 60) driveAnalysis = `<div class="p-2 bg-green-100 rounded-md text-green-800">${translations[currentLang].tachoIncreasedDrive} (${formatDuration(driveTime)})</div>`;
        else driveAnalysis = `<div class="p-2 bg-blue-100 rounded-md text-blue-800">${translations[currentLang].tachoNormalDrive} (${formatDuration(driveTime)})</div>`;
        
        return `<div class="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg">
                    <div class="font-bold">${new Date(record.date).toLocaleDateString(translations[currentLang].locale)}</div>
                    <div class="text-sm space-y-2 mt-2">
                        ${restBefore !== null ? `<div><h4 class="font-semibold">${translations[currentLang].tachoRestBeforeShift}</h4>${restAnalysis}</div>` : ''}
                        <div><h4 class="font-semibold">${translations[currentLang].tachoDailyDriveTime}</h4>${driveAnalysis}</div>
                        <label class="flex items-center gap-2 pt-2 border-t dark:border-gray-700">
                           <input type="checkbox" onchange="handleTachographToggle(this, '${record.id}', 'split')" ${splitRestData[record.id] ? 'checked' : ''}> ${translations[currentLang].tachoSplitRest}
                        </label>
                        <label class="flex items-center gap-2">
                           <input type="checkbox" onchange="handleTachographToggle(this, '${record.id}', 'weekly')" ${weeklyRestData[record.id] ? 'checked' : ''}> ${translations[currentLang].tachoWasWeeklyRest}
                        </label>
                    </div>
                </div>`;
    }).join('');
...
function handlePalletActionChange() {
    ...
    licensePlateContainer.classList.toggle('hidden', action === 'csere');
}
...
async function savePalletEntry() {
    ...
    if (!entry.date || !entry.location || isNaN(entry.quantity) || entry.quantity <= 0) {
        showCustomAlert(i18n.palletInvalidData, 'info');
        return;
    }
    
    if (editingId) {
        palletRecords = palletRecords.map(r => r.id === editingId ? entry : r);
    } else {
        palletRecords.push(entry);
    }
    
    if (currentUser) {
        await db.collection('users').doc(currentUser.uid).collection('pallets').doc(entry.id).set(entry);
    } else {
    ...
    localStorage.setItem('lastPalletLicensePlate', entry.licensePlate);
    renderPalletRecords();
}

function deletePalletEntry(id) {
    showCustomAlert(translations[currentLang].alertConfirmDelete, 'warning', async () => {
        palletRecords = palletRecords.filter(r => r.id !== String(id));
        if (currentUser) {

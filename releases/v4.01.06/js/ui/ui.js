...
function showCustomAlert(message, type, callback, options = {}) {
    ...
    if (type === 'warning') {
        ...
        iconContainer.classList.add('bg-yellow-100');
        iconContainer.innerHTML = warningIcon;
        const okText = (options && options.okText) ? options.okText : i18n.ok;
        buttonsContainer.innerHTML = `<button onclick="hideCustomAlert(true)" class="py-2 w-2/3 bg-yellow-400 text-white rounded-lg font-semibold hover:bg-yellow-500">${okText}</button>`;
        ...
        const okText2 = (options && options.okText) ? options.okText : i18n.ok;
        buttonsContainer.innerHTML = `<button onclick="hideCustomAlert(true)" class="py-2 w-2/3 bg-green-500 text-white rounded-lg font-semibold hover:bg-green-600">${okText2}</button>`;
    }

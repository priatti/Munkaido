...
async function forceCheckForUpdate() {
    ...
    try {
        ...
        // Ha még 4.01.05 alatt futunk, azonnal próbáljunk 4.01.06-ra váltani
        try {
            const hrefNow = String(location.href);
            if (hrefNow.indexOf('/releases/v4.01.05/') !== -1) {
                const next = hrefNow.replace('/releases/v4.01.05/', '/releases/v4.01.06/');
                if (next !== hrefNow) {
                    const i18n2 = (typeof translations !== 'undefined' && translations[(window.currentLang||'hu')]) || {};
                    const notify2 = (title, body, type='info') => {
                    ...
                    notify2('Átváltás új verzióra', 'v4.01.06 betöltése...', 'info');
                    try {
                        if ('serviceWorker' in navigator) {
                            await navigator.serviceWorker.register('/releases/v4.01.06/sw.js');
                        }
                    } catch(_) {}
                    setTimeout(() => { try { location.replace(next); } catch(_) { location.href = next; } }, 150);
                    return;
                }
            }
        } catch(_) {}
        ...
        setTimeout(async () => {
            ...
            // Ha nincs új 05-ös frissítés, próbáljuk meg a migrációt 06-ra
            try {
                const href = String(location.href);
                const next = href.replace('/releases/v4.01.05/', '/releases/v4.01.06/');
                if (next !== href) {
                    notify('Átváltás új verzióra', 'v4.01.06 betöltése...', 'info');
